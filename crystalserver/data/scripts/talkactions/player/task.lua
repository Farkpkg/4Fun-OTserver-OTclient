...
local function sendUsage(player)
	player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Uso: !task list | !task start <id> | !task status | !task check | !task sync")
end
...
function taskCommand.onSay(player, words, param)
	...
	if action == "start" then
		...
		if not taskId then
			player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "Informe o ID. Ex: !task start 1")
			return false
		end
	...
	if action == "status" then
		...
		if activeTaskId == 0 then
			player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Nenhuma task ativa.")
			return false
		end
		...
		player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, string.format("Task ativa [%d] %s: %d/%d", activeTaskId, cfg.name, state.progress, cfg.required))
		return false
	end
	...
	if action == "sync" then
		...
		player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Sincronização enviada ao client.")
		return false
	end
