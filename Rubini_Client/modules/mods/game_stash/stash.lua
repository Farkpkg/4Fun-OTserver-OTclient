...
function retrieveItem(itemId, count, otherWindow)
  g_game.stashWithdraw(itemId, 1, count)
  if countWithdraw then
    countWithdraw:destroy()
    countWithdraw = nil
  end
...
function withdrawItem(widget)
  ...
  countWithdraw.contentPanel.buttonOk.onClick =  function() gameStashWindown:show() retrieveItem(widget:getItemId(), scrollbar:getValue()) end
