...
## 12. Comparativo Estrutural: Battle Pass (COMPLETE_CUSTOM_CLIENT) vs Modelo Oficial Task Board (otclient)

### 12.1 Fontes do modelo oficial em `otclient/`

Módulos encontrados para o modelo oficial de Task Board/Weekly/Bounty:
...
├─ game_weeklytasks.lua
└─ game_weeklytasks.otui

otclient/modules/game_tasks/
├─ tasks.lua
├─ tasks.otui
└─ serverSIDE/extendedopcode.txt
```

### 12.2 Diferenças de arquitetura (visão macro)

| Dimensão | Battle Pass (COMPLETE_CUSTOM_CLIENT) | Task Board oficial (otclient) |
|---|---|---|
| Organização de código | Módulo principal + arquivo de rewards (`battlepass.lua` + `classes/rewards.lua`) | Arquitetura separada por camadas (`taskboard.lua`, `controller`, `viewmodel`, `store`, `protocol`) |
| Estado | Estado global no objeto `BattlePass` | Estado centralizado no `TaskBoardStore` |
| UI | OTUI grande e monolítico (`battlepass.otui`) | OTUI composto por componentes reutilizáveis (`components/*.otui`) |
| Comunicação | Callbacks de alto nível `g_game` (`onBattlePassMissions`, etc.) + chamadas `g_game.*` | ExtendedOpcode JSON explícito (`OPCODE_TASKBOARD=242`, `sync/delta/action`) |
| Evolução incremental | Snapshot-oriented com rerender por funções grandes | Pipeline orientado a `sync + delta` com `reduceDelta()` |
| Acoplamento | Alto acoplamento lógica↔UI por IDs hardcoded | Menor acoplamento via ViewModel e Store |
| Reutilização Task Board | Parcial (widgets e reward-track) | Alta (já modelado para Bounty/Weekly/Shop tabs) |

### 12.3 Padrões do Task Board oficial que faltam no Battle Pass atual

1. **Separação de camadas explícita**
   - TaskBoard oficial separa:
     - transport/protocol (`protocol.lua`),
     - estado (`state_store.lua`),
     - transformação (`taskboard_viewmodel.lua`),
     - apresentação/controlador (`taskboard_controller.lua`).
   - Battle Pass concentra grande parte da orquestração e rendering em um único arquivo.

2. **Contrato de sincronização incremental (`sync` + `delta`)**
   - TaskBoard oficial aceita payload completo (`sync`) e deltas (`delta`).
   - Battle Pass cliente depende de callbacks estruturados, sem reducer explícito.

3. **Composição de UI por componentes**
   - TaskBoard oficial utiliza `components/*.otui` para cartões/sessões.
   - Battle Pass mantém grande parte da estrutura em `battlepass.otui`, dificultando evolução por feature.

4. **Store previsível para mudanças de estado**
   - `TaskBoardStore.reduceDelta()` torna efeitos previsíveis e testáveis.
   - Battle Pass atual atualiza diretamente widgets durante parsing/render.

### 12.4 Mapeamento funcional (equivalência)

| Feature alvo | Battle Pass atual | Task Board oficial |
|---|---|---|
| Missões semanais | Grade de 26 `MissionWidget` + ordenação por `MissionsDisplacement` | Tabs `Bounty`/`Weekly` com grids/cards dedicados |
| Missões diárias | `dailyMissionsBg` (2 slots) | Pode ser modelado como seção dedicada em tab semanal |
| Recompensa por progressão | Track com `RewardPositions` + chest free/premium | Shop + rewards por entrada; pode incorporar progress track |
| Premium | `premiumBattlepass` + lock visual “Deluxe” | Pode virar capability/feature gate no store/viewmodel |
| Reroll | `rerollDailyMission` | Action genérica via opcode/action payload |

### 12.5 Arquitetura ideal para adaptação (proposta)

Objetivo: adaptar Battle Pass para um **Task Board unificado Weekly/Bounty** preservando ativos já prontos.

#### Camadas recomendadas

```text
modules/mods/game_taskboard_unified/
├─ taskboard_unified.lua              # bootstrap (init/terminate/toggle)
├─ protocol/
│  └─ taskboard_protocol.lua          # sendAction + parse sync/delta
├─ store/
│  └─ taskboard_store.lua             # estado imutável/reducer por evento
├─ viewmodel/
│  └─ taskboard_viewmodel.lua         # transforma state -> props de UI
├─ controller/
│  └─ taskboard_controller.lua        # bind UI, handlers de clique
├─ views/
│  ├─ taskboard_unified.otui          # shell principal
│  └─ components/*.otui               # cards/slots/progress/widgets
└─ adapters/
   ├─ battlepass_adapter.lua          # converte payload legado BattlePass
   └─ weeklytasks_adapter.lua         # converte payload Weekly/Bounty
```
...
```json
{
  "type": "sync|delta|error",
  "data": {
    "board": {
      "season": {"id": 2, "begin": 0, "end": 0},
      "difficulty": "Adept",
      "premium": true,
      "points": 0,
      "tabs": {
...
1. **Fase 1 — Anti-corruption layer**
   - Criar `battlepass_adapter.lua` para mapear payload atual (`onBattlePassMissions`, `onBattlePassRewards`) para estado unificado.

2. **Fase 2 — UI híbrida**
   - Reaproveitar assets Battle Pass (`missionBg`, `dailyMissionBg`, chests) dentro de components do novo Task Board.

3. **Fase 3 — Redução de acoplamento**
   - Mover regras de negócio de `configureMissionPanel/configureRewardPanel` para reducer + viewmodel.

4. **Fase 4 — Protocolização única**
   - Padronizar comunicação para ExtendedOpcode JSON com `sync/delta/action`, mantendo fallback para APIs legadas enquanto necessário.

### 12.6 Decisão técnica recomendada

- **Curto prazo:** manter Battle Pass atual e inserir adapter para publicação de estado unificado no Task Board.
- **Médio prazo:** substituir rendering direto por pipeline `store -> viewmodel -> controller`.
- **Longo prazo:** consolidar Battle Pass + Weekly/Bounty + Shop em um único módulo com tabs, reduzindo duplicação de UI e fluxo.

### 12.7 Riscos de adaptação

- Dependência forte dos IDs atuais de OTUI no Battle Pass legado.
- Contrato de servidor possivelmente heterogêneo entre eventos BattlePass e opcodes Weekly/Task.
- Tamanho da `RewardPositions` e comportamento de animação podem exigir componente dedicado para progress track.

### 12.8 Ganhos esperados com a arquitetura ideal

- Menor acoplamento e maior testabilidade.
- Reuso de componentes entre BattlePass/Weekly/Bounty.
- Facilidade de evolução de regras (novos tipos de task/reward).
- Menos risco de regressão visual por centralização de estado e contratos de dados explícitos.
